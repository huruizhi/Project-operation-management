...
#定义docker-compose构成字符串
title_str = '''version: '3'
services:
'''
# module_java_info_str = '''  {project_name}_{module_name}_{stage}:
#     image: harbor.pycf.com/{project_name}/{module_name}:latest
...
'''
    ...
    # module_lists = []
    # module_lists.append(project_info["register"][0])
    # for module in modules:
    #     module_lists.append(module[0])
    print(project_name,modules)
...
'''.format(project_name=register_info[0], module_name=register_info[0], stage=stage, harbor_url=harbor_url,port=register_info[2])
            for module in modules:
                if module[4] == "java":
...
'''.format(project_name=register_info[0], module_name=register_info[0], stage=stage, harbor_url=harbor_url,port=register_info[1])
            for module in modules:
                if module[4] == "java":
                    docker_compose_java_info_str = '''  {project_name}_{module_name}_{stage}:
    image: {harbor_url}/{project_name}/{module_name}:latest
